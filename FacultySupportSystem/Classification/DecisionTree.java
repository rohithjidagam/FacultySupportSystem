...
public class DecisionTree {
	public void createNode(Node node) {
		...
		for (int i = 0; i < numinputattributes; i++) {
			...
			for (int j = 0; j < numvalues; j++) {
				Vector subset = getSubNodes(node.data, i, j);
				if (subset.size() == 0)
					continue;
		...
		for (int j = 0; j < numvalues; j++) {
			...
			node.children[j].data = getSubNodes(node.data, selectedAttribute, j);
			node.children[j].decompositionValue = j;
		}

		for (int j = 0; j < numvalues; j++) {
			createNode(node.children[j]);
		}
		node.data = null;
	}
	...
	public int getValueForTheSymbol(int attribute, String symbol) {
	...
	public Vector getSubNodes(Vector data, int attribute, int value) {
		Vector subset = new Vector();
		int num = data.size();
		for (int i = 0; i < num; i++) {
			Data point = (Data) data.elementAt(i);
	...
	public double calculateEntropy(Vector data) {
		...
		for (int i = 0; i < numvalues; i++) {
			...
			for (int j = 0; j < numdata; j++) {
				Data point = (Data) data.elementAt(j);
	...
	public boolean alreadyUsedToDecompose(Node node, int attribute) {
	...
	public int readData(String filename) throws Exception {
		...
		while (true) {
			...
			if (input == null) {
				System.err
				    .println("No data found in the data file: " + filename + "\n");
				return 0;
			}
			if (input.startsWith("//"))
				continue;
		...
		while (true) {
			...
			Data point = new Data(numAttributes);
			for (int i = 0; i < numAttributes; i++) {
				point.attributes[i] = getValueForTheSymbol(i, tokenizer.nextToken());
			}
